...
class QueryBuilder {
	...
	async insert(items) {
		...
		try {
			...
			if(error.errno === 1062)
				throw new QueryBuilderError(error.code, QueryBuilderError.codes.DUPLICATE_ITEM);

			throw new QueryBuilderError(error.code, QueryBuilderError.codes.MYSQL_DRIVER_ERROR);
		}
