...
class QueryBuilder {
	...
	prepareParams(parameters) {
		...
		// Changed Wildcard for fields
		parameters.fields = this.getFieldsFromWildcard(parameters);
		// If no fields are present
		if(typeof parameters.fields !== 'boolean' && !parameters.fields.length)
	...
	getFieldsFromWildcard(params) {
		...
		if(!fields || !Array.isArray(fields))
			return [];
		if(fields.includes('*'))
			fields = [...fields.filter(field => field !== '*'), ...Object.keys(this.fields).filter(field => !this.fields[field].table)];
