...
class QueryBuilder {
	...
	async save(items) {
		...
		if(!items || !items.length) {
			...
			if(items && typeof items === 'object')
				...
				throw new QueryBuilderError('Not valid items to Save', QueryBuilderError.codes.NO_FIELDS);
		}
		// Format Items to have valid fields
		items = await this._formatFields(items);
	...
	async _getFields() {
		...
		// If Rows format is an array
		if(Array.isArray(rows)) {
			for(const field of rows)
				fields[field.Field] = field;
		} else if(typeof rows === 'object') {
			// If Rows format is an Object
			// eslint-disable-next-line no-restricted-syntax
			for(const field in rows) {
				if(rows[field].Field)
					fields[rows[field].Field] = rows[field];
			}
		}
