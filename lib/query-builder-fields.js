...
class QueryBuilderSelect {
	...
	/**
	 * Search in the extra fields parameters and return fields that have to join on a table
	 * @param {object} parameters parameters
	 * @param {object} modelFields Model fields
	 * @returns {array}
	 */
	static getJoinsExtraFields({ extraFields }, modelFields) {
		if(!extraFields || !Array.isArray(extraFields))
			return [];

		return extraFields.filter(field => (modelFields[field] && modelFields[field].table));
	}
	...
	static buildSelect(knex, model, parameters) {
		...
		// eslint-disable-next-line prefer-destructuring
		const extraFields = parameters.extraFields;
		...
		if(typeof fields === 'undefined') {
			...
			const [selectedFields, selectedFieldFlags] = this._getSelectedFields(fields);
			fieldsForSelect = selectedFields;
			flagFieldsForSelect = selectedFieldFlags;
		...
		// If params has Extra fields
		if(extraFields && Array.isArray(extraFields) && !fields) {
			const [selectedFields, selectedFieldFlags] = this._getSelectedFields(extraFields);

			knex.select(selectedFields);
			flagFieldsForSelect = { ...flagFieldsForSelect, ...selectedFieldFlags };

			nothingToSelect = false;
		}

		if(Object.keys(flagFieldsForSelect).length) {
	...
	/**
	 * Returns Fields for Select along with the flag Fields
	 * @params {array}
	 * @returns {array} [FieldsForSelect, FlagsFieldsForSelect]
	 */
	static _getSelectedFields(fields) {

		const selectedFields = [];

		this._validateFields(fields);

		selectedFields.push(this._getFieldsForSelect(fields));

		selectedFields.push(this._getFlagFieldsForSelect(fields));
